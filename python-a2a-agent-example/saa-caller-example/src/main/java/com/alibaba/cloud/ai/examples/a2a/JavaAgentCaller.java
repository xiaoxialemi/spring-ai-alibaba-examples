...
/**
 * Component for calling Java A2A agents via Nacos discovery.
 *
 ...
 */
@Component
public class JavaAgentCaller {

	private static final Logger logger = LoggerFactory.getLogger(JavaAgentCaller.class);

	private final AgentCardProvider agentCardProvider;

	private final String pythonAgentName;

	public JavaAgentCaller(AgentCardProvider agentCardProvider,
                           @Value("${java.agent.name:JavaEchoAgent}") String pythonAgentName) {
		this.agentCardProvider = agentCardProvider;
		this.pythonAgentName = pythonAgentName;
	}

	/**
	 * Call the Java translator agent.
	 * @param text Text to translate
	 * @return Translation result
	 */
	public String callTranslator(String text) {
		logger.info("Calling Java agent '{}' with text: {}", pythonAgentName, text);
		...
		A2aRemoteAgent remoteAgent = A2aRemoteAgent.builder()
			.name("java-translator-caller") // Local node name
			.description("Caller for Java translator agent")
		...
		try {
			...
			if (result.isPresent()) {
				...
				logger.info("Received response from Java agent: {}", response);
				return response;
			}
			else {
				logger.warn("No response from Java agent");
				return "No response received";
			}
		}
		catch (Exception e) {
			logger.error("Failed to call Java agent", e);
			throw new RuntimeException("Failed to call Java agent: " + e.getMessage(), e);
		}
	}

	/**
	 * Call the Java translator agent with streaming.
	 * @param text Text to translate
	 * @return Translation result
	 */
	public String callTranslatorStreaming(String text) {
		logger.info("Calling Java agent '{}' with streaming, text: {}", pythonAgentName, text);
		...
		A2aRemoteAgent remoteAgent = A2aRemoteAgent.builder()
			.name("java-translator-caller-streaming")
			.description("Streaming caller for Java translator agent")
		...
		try {
			...
			logger.info("Received streaming response from Java agent: {}", response);
			return response.isEmpty() ? "No response received" : response;
		}
		catch (Exception e) {
			logger.error("Failed to call Java agent with streaming", e);
			throw new RuntimeException("Failed to call Java agent: " + e.getMessage(), e);
		}
	}
